...
const RegisterLegalCase = () => {
  ...
  const [titleToCaseTypeMap, setTitleToCaseTypeMap] = useState({});
  const [loadingTitles, setLoadingTitles] = useState(true);
  const [titleFetchError, setTitleFetchError] = useState("");
  ...
  // Fetch case titles and build title-to-caseType mapping
  useEffect(() => {
    const fetchCaseTitles = async () => {
      try {
        const response = await axios.get("http://localhost:8000/caseregister/get-all-cases");
        const allCases = response.data.data;

        // Build list of titles and map to caseType
        const titles = allCases.flatMap(item => item.caseTitles);
        const map = {};
        allCases.forEach(item => {
          item.caseTitles.forEach(title => {
            map[title] = item.caseType;
          });
        });

        setCaseTitles(titles);
        setTitleToCaseTypeMap(map);
      } catch (error) {
        console.error("Failed to fetch case titles:", error);
        setTitleFetchError("Failed to load case titles from server");
        toast.error("Could not load case titles");
      } finally {
  ...
  // Handle case title change and auto-fill caseType
  const handleCaseTitleChange = (e) => {
    const value = e.target.value;
    setFormData((prev) => ({ ...prev, caseTitle: value }));
    validateField("caseTitle", value);

    // Auto-fill caseType
    const caseType = titleToCaseTypeMap[value] || "";
    setFormData((prev) => ({ ...prev, caseType: caseType }));
    validateField("caseType", caseType);
  };
  ...
  const renderStepContent = (step) => {
    switch (step) {
        ...
        return (
          <Box>
            ...
            <FormControl
              ...
              disabled={loadingTitles || titleFetchError}
            >
              <InputLabel id="case-title-label">
                {loadingTitles ? "Loading..." : "Case Title"}
              </InputLabel>
              <Select
                labelId="case-title-label"
                id="case-title-select"
              ...
              >
                ...
                {caseTitles.length === 0 && !loadingTitles && !titleFetchError && (
                  <MenuItem disabled>No case titles available</MenuItem>
                )}
              </Select>
              {errors.caseTitle && (
                <FormHelperText>{errors.caseTitle}</FormHelperText>
              )}
              {titleFetchError && (
                <FormHelperText error>{titleFetchError}</FormHelperText>
              )}
            </FormControl>

            {/* Case Type (Auto-filled, Read-only) */}
            <TextField
              fullWidth
              label="Case Type"
              name="caseType"
      ...
      // Case 1-4: Plaintiff, Defendant, Lawyer, Upload Evidence (unchanged)
      case 1:
              return (
                <Box>
                  <Typography variant="h6" gutterBottom>
